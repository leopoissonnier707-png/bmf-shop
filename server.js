...
app.post('/api/presence', requirePermission('accessPresence'), async (req, res) => {
  const { reason, time, durationHours } = req.body;
  const user = req.session.user;
  if (!reason || !time || !durationHours) return res.status(400).json({ error: 'Champs manquants' });
  ...
  try {
    const msgRes = await axios.post(
      `https://discord.com/api/v10/channels/${PRESENCE_CHANNEL_ID}/messages`,
      {
        ...
        content: '@everyone Réagissez pour indiquer votre présence.'
      },
      { headers: { Authorization: `Bot ${BOT_TOKEN}`, 'Content-Type': 'application/json' } }
    );
    ...
    sendLog('Appel de présence créé', `${reason} — résultats dans ${durationHours}h`, user, 0xcda349);
    res.json({ success: true });
  } catch (err) {
