...
app.get('/api/my-settings', async (req, res) => {
  if (!req.session.user) return res.json({ accentColor: null, fontSize: 'normal' });
  const database = await getDB();
  const s = await database.collection('userSettings').findOne({ discordId: req.session.user.id });
  res.json({ accentColor: s?.accentColor || null, fontSize: s?.fontSize || 'normal' });
});

app.post('/api/my-settings', async (req, res) => {
  if (!req.session.user) return res.status(401).json({ error: 'Non connecté' });
  const { accentColor, fontSize } = req.body;
  const database = await getDB();
