...
async function getBotIdentity() {
  ...
  if (!BOT_TOKEN) return { username: 'Légion', avatar_url: undefined };
  try {
    ...
    return { username: 'Légion', avatar_url: undefined };
  }
}
...
app.post('/api/order', rateLimit(8, 60000), async (req, res) => {
  ...
  const embed = {
    embeds: [{
      title: '🔫 Nouvelle commande Légion',
      color: 0x00d4c8,
      ...
      footer: { text: `Légion · Commande #${order.id} · EN ATTENTE` },
      timestamp: new Date().toISOString()
    }],
...
app.post('/api/order/done', requirePermission('manageOrders'), async (req, res) => {
  ...
  const processedBy = req.session.user ? req.session.user.username : 'Légion';
  ...
  if (BOT_TOKEN && discordId) {
    try {
      ...
      await axios.post(`https://discord.com/api/v10/channels/${channelId}/messages`,
        {
          embeds: [{
            title: '✅ Votre commande Légion est prête !',
            color: 0x4ade80,
            description: `Bonjour **${pseudo}** ! 🎮\n\nVotre commande sur **Légion** est prête.\nTraité par un membre de **Légion** : **${processedBy}**.\nVous serez contacté **en jeu** très prochainement par un membre de **Légion** pour la livraison.\n\nMerci de votre confiance ! 🔫`,
            footer: { text: 'Légion · RP FiveM' },
            timestamp: new Date().toISOString()
          }]
        },
        { headers: { Authorization: `Bot ${BOT_TOKEN}`, 'Content-Type': 'application/json' } }
...
const PERMISSION_KEYS = ['accessAdmin', 'manageWeapons', 'manageOrders', 'managePromos', 'deleteItems', 'accessAbsence', 'accessPresence', 'accessMusic'];
...
app.post('/api/role-permissions', async (req, res) => {
  if (!req.session.isAdmin) return res.status(403).json({ error: 'Non autorisé' });
  const { roleId, roleName, accessAdmin, manageWeapons, manageOrders, managePromos, deleteItems, accessAbsence, accessPresence, accessMusic } = req.body;
  if (!roleId) return res.status(400).json({ error: 'roleId manquant' });
  const database = await getDB();
  await database.collection('rolePermissions').updateOne(
    { roleId },
    { $set: { roleId, roleName, accessAdmin: !!accessAdmin, manageWeapons: !!manageWeapons, manageOrders: !!manageOrders, managePromos: !!managePromos, deleteItems: !!deleteItems, accessAbsence: !!accessAbsence, accessPresence: !!accessPresence, accessMusic: !!accessMusic } },
    { upsert: true }
  );
...
// Récupère qui a le rôle Légion sur le serveur (paginé)
...
app.post('/api/presence/:id/ping', requirePermission('accessPresence'), async (req, res) => {
  ...
  try {
    ...
    const labels = { present: '✅ Présents', late: '⏳ En retard', absent: '❌ Absents', noReact: '❔ N\'ont pas réagi', all: '📣 Tout le monde (@Légion)' };
...
// =====================
// API — MUSIQUE (le site pilote, un bot séparé sur KataBump exécute)
// =====================

// Liste des salons vocaux du serveur
app.get('/api/voice-channels', requirePermission('accessMusic'), async (req, res) => {
  if (!BOT_TOKEN || !DISCORD_GUILD_ID) return res.status(400).json({ error: 'Config manquante' });
  try {
    const r = await axios.get(`https://discord.com/api/v10/guilds/${DISCORD_GUILD_ID}/channels`, { headers: { Authorization: `Bot ${BOT_TOKEN}` } });
    const voiceChannels = r.data.filter(c => c.type === 2).map(c => ({ id: c.id, name: c.name }));
    res.json(voiceChannels);
  } catch (e) {
    console.error('Erreur salons vocaux:', e.response?.data || e.message);
    res.status(500).json({ error: 'Erreur récupération salons vocaux' });
  }
});

// Récupère titre + pochette d'un lien (Spotify via oEmbed public, sinon juste le lien tel quel)
app.post('/api/music/lookup', requirePermission('accessMusic'), async (req, res) => {
  const { url } = req.body;
  ...
  try {
    if (url.includes('open.spotify.com')) {
      ...
      return res.json({ title: r.data.title, thumbnail: r.data.thumbnail_url, url });
    }
    if (url.includes('youtube.com') || url.includes('youtu.be')) {
      const r = await axios.get(`https://www.youtube.com/oembed?url=${encodeURIComponent(url)}&format=json`);
      return res.json({ title: r.data.title, thumbnail: r.data.thumbnail_url, url });
    }
    res.json({ title: url, thumbnail: null, url });
  } catch (e) {
    res.json({ title: url, thumbnail: null, url }); // au pire, on garde juste le lien brut
  }
});

// Playlists personnelles (une par joueur)
app.get('/api/music/playlists', requirePermission('accessMusic'), async (req, res) => {
  const database = await getDB();
  const list = await database.collection('playlists').find({ ownerId: req.session.user.id }).toArray();
  res.json(list);
});

app.post('/api/music/playlists', requirePermission('accessMusic'), async (req, res) => {
  const { name } = req.body;
  if (!name) return res.status(400).json({ error: 'Nom manquant' });
  const database = await getDB();
  const result = await database.collection('playlists').insertOne({ ownerId: req.session.user.id, name, tracks: [], createdAt: new Date().toISOString() });
  res.json({ success: true, id: result.insertedId });
});

app.delete('/api/music/playlists/:id', requirePermission('accessMusic'), async (req, res) => {
  const { ObjectId } = require('mongodb');
  let objId; try { objId = new ObjectId(req.params.id); } catch(e) { return res.status(400).json({ error: 'ID invalide' }); }
  const database = await getDB();
  await database.collection('playlists').deleteOne({ _id: objId, ownerId: req.session.user.id });
  res.json({ success: true });
});

app.post('/api/music/playlists/:id/tracks', requirePermission('accessMusic'), async (req, res) => {
  const { ObjectId } = require('mongodb');
  let objId; try { objId = new ObjectId(req.params.id); } catch(e) { return res.status(400).json({ error: 'ID invalide' }); }
  const { title, thumbnail, url } = req.body;
  const database = await getDB();
  await database.collection('playlists').updateOne(
    { _id: objId, ownerId: req.session.user.id },
    { $push: { tracks: { title, thumbnail, url, addedAt: new Date().toISOString() } } }
  );
  res.json({ success: true });
});

app.delete('/api/music/playlists/:id/tracks/:index', requirePermission('accessMusic'), async (req, res) => {
  const { ObjectId } = require('mongodb');
  let objId; try { objId = new ObjectId(req.params.id); } catch(e) { return res.status(400).json({ error: 'ID invalide' }); }
  const database = await getDB();
  const pl = await database.collection('playlists').findOne({ _id: objId, ownerId: req.session.user.id });
  if (!pl) return res.status(404).json({ error: 'Playlist introuvable' });
  pl.tracks.splice(parseInt(req.params.index), 1);
  await database.collection('playlists').updateOne({ _id: objId }, { $set: { tracks: pl.tracks } });
  res.json({ success: true });
});

// Envoie une commande au bot musical (qui tourne séparément sur KataBump et surveille cette collection)
app.post('/api/music/command', requirePermission('accessMusic'), async (req, res) => {
  const { action, voiceChannelId, playlistId, mode } = req.body; // action: 'play' | 'skip' | 'stop' | 'pause' | 'resume'
  if (!action) return res.status(400).json({ error: 'Action manquante' });
  const database = await getDB();
  let tracks = [];
  if (playlistId && action === 'play') {
    const { ObjectId } = require('mongodb');
    try {
      const pl = await database.collection('playlists').findOne({ _id: new ObjectId(playlistId) });
      if (pl) {
        tracks = pl.tracks.slice();
        if (mode === 'shuffle') tracks.sort(() => Math.random() - 0.5);
        if (mode === 'reverse') tracks.reverse();
      }
    } catch(e) {}
  }
  await database.collection('musicCommands').insertOne({
    action, voiceChannelId: voiceChannelId || null, guildId: DISCORD_GUILD_ID,
    tracks, requestedBy: req.session.user, createdAt: new Date().toISOString(), processed: false
  });
  sendLog('Commande musique envoyée', `${action}${voiceChannelId ? ' — salon ' + voiceChannelId : ''}${tracks.length ? ' — ' + tracks.length + ' titre(s)' : ''}`, req.session.user, 0xcda349);
  res.json({ success: true });
});

// État actuel de la lecture (mis à jour par le bot lui-même sur KataBump)
app.get('/api/music/status', requirePermission('accessMusic'), async (req, res) => {
  const database = await getDB();
  const status = await database.collection('musicState').findOne({ guildId: DISCORD_GUILD_ID });
...
initDB().then(() => {
  app.listen(PORT, () => {
    console.log(`Légion démarré sur le port ${PORT}`);
  });
}).catch(err => {
