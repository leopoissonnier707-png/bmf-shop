    ...
    for (const channel of textChannels) {
      ...
      for (let page = 0; page < 20; page++) { // limite de sécurité : 20 pages de 100 messages max par salon
        ...
        for (let i = 0; i < bulkable.length; i += 100) {
          ...
          if (chunk.length === 1) {
            try { await axios.delete(`https://discord.com/api/v10/channels/${channel.id}/messages/${chunk[0]}`, { headers: { Authorization: `Bot ${BOT_TOKEN}` } }); deletedMessages++; chDeleted++; } catch(e) { errors++; chErrorReason = e.response?.data?.message || e.message; }
        ...
        for (const id of old) {
          try { await axios.delete(`https://discord.com/api/v10/channels/${channel.id}/messages/${id}`, { headers: { Authorization: `Bot ${BOT_TOKEN}` } }); deletedMessages++; chDeleted++; } catch(e) { errors++; chErrorReason = e.response?.data?.message || e.message; }
