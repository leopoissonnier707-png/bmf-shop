...
app.post('/api/presence', requirePermission('accessPresence'), async (req, res) => {
  const { reason, time, durationHours, roleId, roleName } = req.body;
  const user = req.session.user;
  if (!reason || !time || !durationHours || !roleId) return res.status(400).json({ error: 'Champs manquants' });
  ...
  try {
      ...
      `https://discord.com/api/v10/channels/${PRESENCE_CHANNEL_ID}/messages`,
      {
        embeds: [{
          ...
          fields: [
            ...
            { name: '⏱️ Résultats dans', value: `${durationHours}h`, inline: true },
    ...
    await database.collection('presenceEvents').insertOne({
      messageId, channelId: PRESENCE_CHANNEL_ID, reason, time, durationHours: parseFloat(durationHours),
      roleId, roleName: roleName || 'Rôle',
      createdBy: user, createdAt: new Date().toISOString(),
      closeAt: new Date(Date.now() + parseFloat(durationHours) * 60 * 60 * 1000).toISOString(),
      resultsPosted: false
    });

    sendLog('Appel de présence créé', `${reason} — résultats dans ${durationHours}h — rôle ${roleName || roleId}`, user, 0xcda349);
    res.json({ success: true });
  } catch (err) {
