...
app.post('/api/absence', requirePermission('accessAbsence'), async (req, res) => {
  const { rpName, from, to, signature } = req.body;
  const user = req.session.user;
  if (!rpName || !from || !to || !signature) return res.status(400).json({ error: 'Champs manquants' });
  ...
  try {
    await database.collection('absences').insertOne({
      discordId: user.id, discordUser: user, rpName, from, to, signature, createdAt: entry.createdAt
    });
